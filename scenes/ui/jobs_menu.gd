...
var _available_jobs: Array[JobData] = []
...
func _populate_list() -> void:
	...
	for job in _available_jobs:
		...
		item.text = "%s → %s ($%d)" % [job.display_name, job.get_target_label(), job.money_reward]
		item.job_data = job
...
func _refresh_carrying() -> void:
	...
	for job in GameState.active_jobs:
		var label := Label.new()
		label.text = "%s → %s" % [job.display_name, job.get_target_label()]
